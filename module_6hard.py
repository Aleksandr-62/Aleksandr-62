...
class Figure:   # Класс фигуры
    ...
    def __init__(self, color, sides):
        self.__sides = sides    # список сторон (целые числа (int))
        self.__color = color    # (список цветов в формате RGB (int))
    ...
    def __is_valid_sides(self, new_sides):      # принимает неограниченное кол-во сторон,
        ...
        if self.sides_count != len(new_sides):  # и кол-во новых сторон совпадает с текущим, False - во всех
            return False                        # остальных случаях.
    ...
    def set_sides(self, *new_sides):            # должен принимать новые стороны, если их количество не равно
        if self.__is_valid_sides(new_sides):    # sides_count, то не изменять, в противном случае - менять.
