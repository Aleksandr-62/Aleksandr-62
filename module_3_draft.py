...
#
# # __ ф-ция Подсчет четных чисел в списке ___
#
...
#     return couter
#
# print(count_even([2, 4, 3, 5, 6, 7, 9]))
#
...
def sub():  # __ ф-ция для кнопки "-"
    num1, num2 = get_values()
    res = num1 - num2
    insert_values(res)


def mul():  # __ ф-ция для кнопки "*"
    num1, num2 = get_values()
    res = num1 * num2
    insert_values(res)


def div():  # __ ф-ция для кнопки "/"
    num1, num2 = get_values()
    res = num1 / num2
    insert_values(res)
...
button_sub = tk.Button(window, text='-', height=2, command=sub)
button_sub.place(x=150, y=200)
button_mul = tk.Button(window, text='*', height=2, command=mul)
button_mul.place(x=200, y=200)
button_div = tk.Button(window, text='/', height=2, command=div)
...
number1 = tk.Label(window, text='Введите первое число')  # Виджет надпись
